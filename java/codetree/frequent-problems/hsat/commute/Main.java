...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        boolean[] sToMid = getReachableSet(S, T, edges, S);
        boolean[] midToT = getReachableSet(T, S, reverseEdges, S);
        // for (int i = 0; i < n + 1; i++) {
        ...
        boolean[] tToMid = getReachableSet(T, S, edges, T);
        boolean[] midToS = getReachableSet(S, T, reverseEdges, T);
        // for (int i = 0; i < n + 1; i++) {
    ...
    private static boolean[] getReachableSet(int startVertex, int endVertex, List<List<Integer>> edges,
        ...
        while (!queue.isEmpty()) {
            ...
            if (cur == endVertex) {
                // 출발 지점은 다시 방문 가능
                if (endVertex == realStartVertex && realStartVertexCnt == 0) {
                    realStartVertexCnt++;
                } else {
                    continue;
                }
